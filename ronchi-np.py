...
def np_vec_dot(a, b):
    return np.sum(a * b, axis=-1) # Sum along the last axis for dot product of N vectors
...
def np_vec_normalize(a):
    ...
    l_safe = np.where(l == 0, 1.0, l) # Use 1.0 for division if length is 0, then multiply by 0 later
    normalized_a = a / l_safe[..., np.newaxis] # Ensure broadcasting aligns
    normalized_a = np.where(l[..., np.newaxis] == 0, 0.0, normalized_a) # Set to 0 if original length was 0
    return normalized_a
...
def main():
    ...
    parser.add_argument('-d', '--diameter', type=float, help='Diameter (default: %(default)s)', default=12.5)
    parser.add_argument('-f', '--focal-length', type=float, help='Focal length (default: %(default)s)', default=62.5)
    parser.add_argument('-g', '--grating-frequency', type=float, 
            help='Grating frequency (default: %(default)s)', default=85.)
    parser.add_argument('-o', '--offset', type=float, help='Offset (default: %(default)s)', default=-0.25)
    parser.add_argument('-k', '--conic-constant', type=float, help='Conic constant K (default: -1.0)', default=-1)
    parser.add_argument('--out', type=str, default=output_filename,
    ...
    if np.any(parabola_mask):
        ...
        if parabola_mask: # K is -1 (scalar)
            ...
            P_z_conic_val[invalid_discriminant_mask] = np.nan # Use NaN to mark invalid points
            P_z = P_z_conic_val
    ...
    t_values[valid_denom_mask] = numerator[valid_denom_mask] / R_z_values[valid_denom_mask]
    ...
    ronchi_pattern_mask = (floor_gx % 2) != 0 # Odd -> True (white), Even -> False (gray)
    ...
    try:
        ...
        i1.text((0, 16), f'{abs(offset)} inch {"inside" if offset < 0 else "outside"} of focus', fill=(255), font=font)
